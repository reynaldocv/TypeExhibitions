...
while($o_items && $o_items->nextHit()) {    
  ...
  $typo_id = $o_items ->getWithTemplate('^ca_occurrences.tipo_exposicao', array('locale' => 'en_US'));
    ...
    <tr>
      ...
      <td>
        <?php 
          print "$name" ;
        ?>
        <input type='hidden' id="type-<?php print $cnt ?>" value="<?php print $typo_id; ?>"/>
        <input type='hidden' id="idno-<?php print $cnt ?>" value="<?php print $id; ?>"/>
      </td>
      <td>        
        <?php //print "<div id='wikicode-$cnt'> $tipo_id - $tipo_labels </div>";  ?>      
      </td>
      <td>
  ...
  <script>
    ...
    function roboto(idx)
    {
      if (idx <= limit){
        ...
        var type = document.querySelector("#type-" + idx.toString()).value.trim();
